...
router.get('/candidates', authenticateToken, async (req, res) => {
  try {
    ...
    if (skills) {
      ...
      filter['skillsAndCertifications.technicalSkills.skillName'] = { $in: skillsArray };
    }
    ...
    if (company) {
      filter['professionalInfo.currentCompany'] = { $regex: company, $options: 'i' };
    }
    ...
    const transformedCandidates = candidateProfiles.map(profile => {
      return {
        ...
        name: profile.personalInfo ? `${profile.personalInfo.firstName} ${profile.personalInfo.lastName}` : 'Unknown',
        ...
        skills: profile.skillsAndCertifications?.technicalSkills?.map(s => s.skillName) || [],
        ...
        isFresher: !profile.professionalInfo?.totalExperience || profile.professionalInfo.totalExperience === '0' || profile.professionalInfo.totalExperience.includes('0'),
        ...
        graduationYear: profile.educationHistory?.graduation?.yearOfPassing || '',
        ...
        // Certifications
        certifications: profile.skillsAndCertifications?.certifications?.map(c => 
          typeof c === 'string' ? c : c.name
        ).filter(Boolean) || [],
        certificationsDetail: profile.skillsAndCertifications?.certifications || [],
    ...
    console.log(`📊 Fetched ${candidateProfiles.length} candidates from database (Total: ${total})`);
...
router.get('/candidates/:candidateId', authenticateToken, async (req, res) => {
  try {
    ...
    const candidateProfile = await CandidateProfile.findById(req.params.candidateId);
    ...
    const transformedCandidate = {
      ...
      name: candidateProfile.personalInfo ? `${candidateProfile.personalInfo.firstName} ${candidateProfile.personalInfo.lastName}` : 'Unknown',
      ...
      position: candidateProfile.professionalInfo?.currentRole || 'Not specified',
      location: candidateProfile.personalInfo?.address?.city || '',
      skills: candidateProfile.skillsAndCertifications?.technicalSkills.map(s => s.skillName) || [],
      status: 'Profile Complete', // Default status
      matchScore: 0, // Placeholder, can be calculated if needed
      ...
      mobile: candidateProfile.professionalInfo?.mobile || candidateProfile.personalInfo?.phone || '',
      ...
      relevantExperience: candidateProfile.professionalInfo?.relevantExperience || '',
      isFresher: !candidateProfile.professionalInfo?.totalExperience || parseFloat(candidateProfile.professionalInfo?.totalExperience) === 0,
      education: candidateProfile.educationHistory?.graduation?.degree || '',
      degree: candidateProfile.educationHistory?.graduation?.degree || '',
      university: candidateProfile.educationHistory?.graduation?.university || '',
      graduationYear: candidateProfile.educationHistory?.graduation?.yearOfPassing || '',
      ...
      academicDetails: {
        ...
        additionalEducation: candidateProfile.educationHistory?.additionalEducation || ''
      },
      ...
      workAuthorization: candidateProfile.professionalInfo?.workAuthorization || '',
      availability: candidateProfile.professionalInfo?.availability || '',
      preferredLocation: candidateProfile.workPreferences?.preferredLocation || [],
      workPreferences: candidateProfile.workPreferences?.workPreferences || [],
      languages: candidateProfile.skillsAndCertifications?.languages.map(l => l.language) || [],
      certifications: candidateProfile.skillsAndCertifications?.certifications.map(c => c.certificationName) || [],
...
router.put("/applications/:id/:action", authenticateToken, async (req, res) => {
  try {
    ...
    let newStatus;
    switch (action) {
      case "review": newStatus = "reviewing"; break;
      case "shortlist": newStatus = "shortlisted"; break;
      case "reject": newStatus = "rejected"; break;
      case "schedule-interview": newStatus = "interview-scheduled"; break;
      case "hire": newStatus = "hired"; break;
...
async function updateJobStatistics(jobId) {
  try {
    ...
    const statsMap = {};
    stats.forEach((stat) => { statsMap[stat._id] = stat.count; });
