  ...
  const StatCard = ({
    ...
    <div className="group bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-md rounded-3xl p-8 border border-white/20 hover:border-white/40 transition-all duration-500 hover:scale-105 hover:shadow-2xl">
      <div className="flex items-center justify-between mb-6">
        <div
          className={`text-5xl p-4 bg-gradient-to-br ${color === "primary" ? "from-red-400/20 to-red-600/20" : color === "secondary" ? "from-blue-400/20 to-blue-600/20" : "from-purple-400/20 to-purple-600/20"} rounded-2xl backdrop-blur-sm group-hover:scale-110 transition-transform duration-300`}
        >
          {cardLoading ? "⏳" : icon}
        </div>
        {trend && (
          <div
            className={`flex items-center space-x-2 px-3 py-1 rounded-full ${trend > 0 ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400"}`}
          >
      ...
      <div
        className={`text-4xl font-bold mb-2 ${color === "primary" ? "bg-gradient-to-r from-red-400 to-red-600" : color === "secondary" ? "bg-gradient-to-r from-blue-400 to-blue-600" : "bg-gradient-to-r from-purple-400 to-purple-600"} bg-clip-text text-transparent`}
      >
        {cardLoading
          ? "..."
          : typeof value === "number"
      ...
      <div className="mt-4 h-1 bg-gray-700 rounded-full overflow-hidden">
        <div
          ...
          style={{
            width: cardLoading
              ? "0%"
              : `${Math.min((typeof value === "number" ? value : 0) / Math.max((typeof value === "number" ? value : 0) + 20, 100), 100) * 100}%`,
          }}
        ></div>
      </div>
    </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 relative overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div
          className="absolute top-40 right-20 w-96 h-96 bg-gradient-to-br from-blue-400/10 to-blue-600/10 rounded-full blur-3xl animate-pulse"
          style={{ animationDelay: "2s" }}
        ></div>
        <div
          className="absolute bottom-20 left-1/3 w-80 h-80 bg-gradient-to-br from-red-400/10 to-blue-600/10 rounded-full blur-3xl animate-pulse"
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-2 sm:px-4 lg:px-6 pt-2 pb-6 space-y-12">
        ...
        <div className="text-center py-12">
          <div className="animate-fade-in-up">
            ...
            <p className="text-lg md:text-xl text-gray-300 max-w-4xl mx-auto font-light leading-relaxed mb-6">
              Welcome back, {user?.firstName}! Here's your intelligent ATS
              command center
            </p>
              ...
              <div className="space-y-4 max-h-96 overflow-y-auto">
                {dashboardData.recentActivity.length > 0 ? (
                  dashboardData.recentActivity.map((activity, index) => (
                    <div
                      key={activity.id}
                      className={`group bg-white/5 backdrop-blur-sm rounded-2xl p-6 border-l-4 transition-all duration-300 hover:bg-white/10 hover:scale-[1.02] ${getPriorityColor(activity.priority)}`}
                    >
                      ...
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            ...
                            <div
                              className={`inline-block px-3 py-1 rounded-full text-xs font-medium text-white mt-1 ${getStatusColor(candidate.status)}`}
                            >
                              {candidate.status}
                            </div>
                          </div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {candidate.skills
                          ...
                          .map((skill, skillIndex) => (
                            <span
                              key={skillIndex}
                              className="px-2 py-1 bg-blue-500/20 text-blue-300 text-xs rounded-lg border border-blue-500/30 hover:border-blue-400/50 transition-colors duration-200"
                            >
                              {skill}
                            </span>
                          ))}
        ...
        <div className="bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-md rounded-3xl p-8 border border-white/20">
          <h3 className="text-3xl font-bold text-white mb-8 text-center">
            💼 Acti ve Jobs Overview
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {dashboardData.jobsOverview.length > 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="text-center p-6 bg-white/5 rounded-2xl border border-white/10">
              ...
              <div className="text-2xl font-bold text-red-400 mb-2">
                {dashboardData.systemMetrics.processingSpeed}/hr
              </div>
            ...
            <div className="text-center p-6 bg-white/5 rounded-2xl border border-white/10">
              ...
              <div className="text-2xl font-bold text-green-400 mb-2">
                {dashboardData.systemMetrics.matchAccuracy}%
              </div>
        ...
        <div className="bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-md rounded-3xl p-8 border border-white/20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              ...
              >
                <div
                  className={`absolute inset-0 bg-gradient-to-r ${action.color === "red" ? "from-red-600/30 to-red-400/30" : "from-blue-600/30 to-blue-400/30"} rounded-xl blur-sm group-hover:blur-md transition-all duration-300`}
                ></div>
                <div
                  className={`relative flex flex-col items-center p-8 bg-gradient-to-br ${action.color === "red" ? "from-red-500/20 to-red-600/20 border-red-400/50" : "from-blue-500/20 to-blue-600/20 border-blue-400/50"} backdrop-blur-sm rounded-xl border border-white/20 hover:border-opacity-50 transition-all duration-300 hover:scale-105 hover:rotate-1 group-hover:shadow-2xl`}
                >
