...
export default function CandidatesPage() {
  ...
  const calculateStats = (candidatesData) => {
    ...
    const newCount = candidatesData.filter(
      (c) => c.status === "new" || c.status === "pending",
    ).length;
    const screening = candidatesData.filter(
      (c) => c.status === "screening" || c.status === "reviewing",
    ).length;
    const interview = candidatesData.filter(
      (c) => c.status === "interview" || c.status === "interview-scheduled",
    ).length;
    const offer = candidatesData.filter((c) => c.status === "offer").length;
    const hired = candidatesData.filter((c) => c.status === "hired").length;
    const rejected = candidatesData.filter(
      (c) => c.status === "rejected",
    ).length;
    const avgMatch =
      total > 0
        ? Math.round(
            candidatesData.reduce((sum, c) => sum + (c.matchScore || 0), 0) /
              total,
          )
        : 0;
  ...
  const updateCandidateStatus = async (candidateId, newStatus, notes = "") => {
    try {
      const response = await apiCall(
        `/api/admin/applications/${candidateId}/${getActionFromStatus(newStatus)}`,
        {
          method: "PUT",
  ...
  const filteredAndSortedCandidates = candidates
    ...
    .sort((a, b) => {
      switch (sortBy) {
        ...
        case "date":
          return (
            new Date(b.appliedDate || a.createdAt) -
    ...
    <div className="group bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-md rounded-3xl p-8 border border-white/20 hover:border-white/40 transition-all duration-500 hover:scale-105">
      <div className="flex justify-between items-start mb-6">
        <div className="flex items-center space-x-6">
          <div className="relative">
            {candidate.profilePicture?.fileUrl ? (
              <img
                src={
                  candidate.profilePicture.fileUrl.startsWith("http")
                    ? candidate.profilePicture.fileUrl
                    : candidate.profilePicture.fileUrl
                }
                alt={candidate.name}
          ...
          <div>
            ...
            <div className="flex items-center space-x-2 mb-3">
              <span
                className={`px-3 py-1 rounded-full text-xs font-medium ${candidate.isFresher ? "bg-green-500/20 text-green-300 border border-green-500/30" : "bg-blue-500/20 text-blue-300 border border-blue-500/30"}`}
              >
                {candidate.isFresher
                  ? `Fresher`
            ...
            <div className="grid grid-cols-2 gap-4 text-sm text-gray-400">
              ...
              <div className="flex items-center space-x-1">
                <span>📱</span>
                <span>
                  {candidate.mobile || candidate.phone || "Phone not provided"}
                </span>
              </div>
              <div className="flex items-center space-x-1">
                <span>📧</span>
              ...
              <div className="flex items-center space-x-1">
                <span>🏢</span>
                <span>
                  {candidate.currentCompany || "Current company not specified"}
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="flex flex-col items-end space-y-3">
          <div className="text-center">
            <div
              className={`text-4xl font-bold ${getScoreColor(candidate.atsMatch || candidate.matchScore || 0)}`}
            >
          ...
          <div
            className={`px-4 py-2 rounded-full text-sm font-medium text-white ${getStatusColor(candidate.status)}`}
          >
            {formatStatus(candidate.status).charAt(0).toUpperCase() +
              formatStatus(candidate.status).slice(1)}
          </div>
        </div>
      </div>
      ...
      <div className="mb-6">
        <p className="text-gray-300 leading-relaxed">
          {candidate.summary || candidate.bio || "No summary available"}
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-6 mb-6">
        <div>
          ...
          <ul className="text-sm text-gray-300 space-y-1">
            {(candidate.strengths || candidate.aiAnalysis?.strengths || []).map(
              (strength, index) => (
        ...
        <div>
          ...
          <ul className="text-sm text-gray-300 space-y-1">
            {(candidate.concerns || candidate.aiAnalysis?.weaknesses || []).map(
              (concern, index) => (
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6 text-center">
        <div className="bg-white/5 rounded-xl p-3 border border-white/10">
          ...
          <div className="text-white font-medium text-sm">
            {candidate.academicDetails?.degree ||
              candidate.education ||
              "Not specified"}
          </div>
          <div className="text-xs text-gray-500">
            {candidate.academicDetails?.university || ""}
          </div>
        </div>
        <div className="bg-white/5 rounded-xl p-3 border border-white/10">
      ...
      <div className="flex space-x-4">
        ...
        {candidate.resume?.fileUrl && (
          <a
            href={
              candidate.resume.fileUrl.startsWith("http")
                ? candidate.resume.fileUrl
                : candidate.resume.fileUrl
            }
            target="_blank"
        ...
        {candidate.status === "pending" && (
          <button
            onClick={() =>
              updateCandidateStatus(candidate._id || candidate.id, "reviewing")
            }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 relative overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div
          className="absolute top-1/3 right-10 w-80 h-80 bg-gradient-to-br from-pink-400/8 to-red-600/8 rounded-full blur-3xl animate-pulse"
          style={{ animationDelay: "2s" }}
        ></div>
        <div
          className="absolute bottom-20 left-1/4 w-72 h-72 bg-gradient-to-br from-green-400/8 to-cyan-600/8 rounded-full blur-3xl animate-pulse"
              ...
              <div className="flex items-center space-x-4">
                <div className="relative">
                  {selectedCandidate.profilePicture?.fileUrl ? (
                    <img
                      src={
                        selectedCandidate.profilePicture.fileUrl.startsWith(
                          "http",
                        )
                          ? selectedCandidate.profilePicture.fileUrl
                          : selectedCandidate.profilePicture.fileUrl
                ...
                <div>
                  ...
                  <div className="flex items-center space-x-2 mt-2">
                    <span
                      className={`px-3 py-1 rounded-full text-sm font-medium ${getStatusColor(selectedCandidate.status)}`}
                    >
                      {formatStatus(selectedCandidate.status)}
                    </span>
                    <span
                      className={`text-2xl font-bold ${getScoreColor(selectedCandidate.atsMatch || selectedCandidate.matchScore || 0)}`}
                    >
                      {selectedCandidate.atsMatch ||
                ...
                <div className="space-y-3 text-sm">
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <span className="text-white">
                      {selectedCandidate.firstName || "N/A"}
                    </span>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <span className="text-gray-400">Last Name:</span>
                    <span className="text-white">
                      {selectedCandidate.lastName || "N/A"}
                    </span>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <span className="text-white">
                      {selectedCandidate.alternatePhone || "N/A"}
                    </span>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
              ...
              <div className="bg-white/5 rounded-xl p-6 border border-white/10">
                ...
                <div className="space-y-3 text-sm">
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <span className="text-white">
                      {selectedCandidate.address?.street || "N/A"}
                    </span>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <span className="text-white">
                      {selectedCandidate.address?.state || "N/A"}
                    </span>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <span className="text-gray-400">Postal Code:</span>
                    <span className="text-white">
                      {selectedCandidate.address?.postalCode || "N/A"}
                    </span>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <span className="text-gray-400">Country:</span>
                    <span className="text-white">
                      {selectedCandidate.address?.country || "India"}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white/5 rounded-xl p-6 border border-white/10">
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <span className="text-white">
                      {selectedCandidate.totalExperience ||
                        (selectedCandidate.isFresher ? "Fresher" : "N/A")}
                    </span>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <span className="text-gray-400">Relevant Experience:</span>
                    <span className="text-white">
                      {selectedCandidate.relevantExperience || "N/A"}
                    </span>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <span className="text-white">
                      {selectedCandidate.workAuthorization || "N/A"}
                    </span>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <span className="text-gray-400">Availability:</span>
                    <span className="text-white">
                      {selectedCandidate.availability || "N/A"}
                    </span>
                  </div>
                </div>
              </div>
                ...
                <div className="space-y-3 text-sm">
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <span className="text-white">
                      {selectedCandidate.degree ||
                        selectedCandidate.academicDetails?.degree ||
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <span className="text-white text-xs">
                      {selectedCandidate.university ||
                        selectedCandidate.academicDetails?.university ||
                        "N/A"}
                    </span>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <span className="text-white">
                      {selectedCandidate.graduationYear ||
                        selectedCandidate.academicDetails?.graduationYear ||
                        "N/A"}
                    </span>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <span className="text-white">
                      {selectedCandidate.gpa ||
                        selectedCandidate.academicDetails?.gpa ||
                        "N/A"}
                    </span>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <span
                      className={
                        selectedCandidate.abcId ||
                        selectedCandidate.academicDetails?.abcId
                          ? "text-green-400"
                          : "text-gray-500"
                      }
                    >
                  ...
                  <div className="col-span-2">
                    ...
                    <span className="text-white text-xs leading-relaxed">
                      {selectedCandidate.education ||
                        selectedCandidate.academicDetails
                          ?.additionalEducation ||
                ...
                <div className="space-y-3 text-sm">
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {(selectedCandidate.preferredLocation || []).length >
                      0 ? (
                        selectedCandidate.preferredLocation.map(
                          (location, index) => (
                            <span
                              key={index}
              ...
              <div className="bg-white/5 rounded-xl p-6 border border-white/10 mb-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedCandidate.certifications.map((cert, index) => (
                    <span
                      key={index}
                      className="px-3 py-1 bg-orange-500/20 text-orange-300 text-sm rounded-lg border border-orange-500/30"
                    >
                      {cert}
                    </span>
                  ))}
              ...
              <div className="bg-white/5 rounded-xl p-6 border border-white/10 mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                  <div>
                    ...
                    <span className="text-white">
                      {selectedCandidate.resume.fileName || "N/A"}
                    </span>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    {selectedCandidate.resume.fileUrl && (
                      <a
                        href={
                          selectedCandidate.resume.fileUrl.startsWith("http")
                            ? selectedCandidate.resume.fileUrl
                            : selectedCandidate.resume.fileUrl
                        }
                        target="_blank"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div className="bg-white/5 rounded-xl p-6 border border-white/10">
                  ...
                  <ul className="space-y-2">
                    {(
                      selectedCandidate.concerns ||
                      selectedCandidate.aiAnalysis?.weaknesses ||
                      []
                    ).map((concern, index) => (
                      <li key={index} className="text-gray-300 text-sm">
                        • {concern}
            ...
            <div className="bg-white/5 rounded-xl p-6 border border-white/10 mb-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm">
                ...
                <div>
                  ...
                  <span className="text-white">
                    {selectedCandidate.applicationSource ||
                      "Direct Application"}
                  </span>
                </div>
                <div>
                  ...
                  <span
                    className={`font-bold ${getScoreColor(selectedCandidate.atsMatch || selectedCandidate.matchScore || 0)}`}
                  >
                    {selectedCandidate.atsMatch ||
                      selectedCandidate.matchScore ||
                      0}
