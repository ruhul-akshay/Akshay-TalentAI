...
export default function ATSPage() {
  ...
  const analyzeResumes = async () => {
    ...
    try {
      ...
      const genericJobDesc =
        "Analyze this resume and extract all relevant information including personal details, skills, experience, and education.";
  ...
  const confirmAndSubmit = async () => {
    try {
      ...
      setMessage({
        type: "error",
        text: `Failed to add candidate: ${error.message}`,
      });
    }
  };
  ...
  const calculateATSScore = async (candidateId) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setMessage({
          type: "success",
          text: "ATS score calculated successfully!",
        });
      } else {
      ...
      setMessage({
        type: "error",
        text: `Failed to calculate ATS score: ${error.message}`,
      });
    } finally {
      setAnalysisLoading(false);
    }
  ...
  const updateCandidateStatus = async (candidateId, newStatus) => {
    try {
      const response = await apiCall(
        `/api/applications/${candidateId}/${getActionFromStatus(newStatus)}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
  ...
  const filteredCandidates = jobApplications
    ...
    .sort((a, b) => {
      switch (sortBy) {
        case "matchScore":
          return (
            (b.aiAnalysis?.matchPercentage || 0) -
            (a.aiAnalysis?.matchPercentage || 0)
          );
        case "name":
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-12">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-lg md:text-xl text-gray-300 max-w-4xl mx-auto font-light leading-relaxed">
            Intelligent resume analysis and job-based candidate matching powered
            by AI
          </p>
        </div>
              ...
              <div className="space-y-6">
                <div className="bg-gradient-to-r from-green-500/10 to-blue-500/10 backdrop-blur-md rounded-2xl p-4 border border-green-500/20">
                  ...
                  <p className="text-green-300 mt-2">
                    Review extracted data and confirm to add to database
                  </p>
                </div>
                ...
                {analyzedCandidates.map((candidate, index) => (
                  <div
                    key={index}
                    className="bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-md rounded-3xl p-6 border border-white/20"
                  >
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                          <div>
                            ...
                            <span className="text-white font-medium">
                              {candidate.extractedInfo?.name || "Not found"}
                            </span>
                          </div>
                          <div>
                            <span className="text-gray-400 block">Email:</span>
                            <span className="text-white">
                              {candidate.extractedInfo?.email || "Not found"}
                            </span>
                          </div>
                          <div>
                            <span className="text-gray-400 block">Phone:</span>
                            <span className="text-white">
                              {candidate.extractedInfo?.phone || "Not found"}
                            </span>
                          </div>
                          <div>
                            ...
                            <span className="text-white">
                              {candidate.extractedInfo?.totalYearsExperience ||
                                "Not specified"}
                            </span>
                          </div>
                        </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                    ...
                    <div className="bg-gradient-to-br from-purple-500/20 to-purple-600/20 rounded-2xl p-4 text-center border border-purple-500/30">
                      <div className="text-2xl font-bold text-purple-400">
                        {
                          jobApplications.filter(
                            (app) => app.aiAnalysis?.matchPercentage >= 80,
                          ).length
                        }
                      </div>
                      <div className="text-sm text-gray-300">
                    ...
                    >
                      <div className="flex items-start justify-between mb-6">
                        ...
                        <div className="flex items-center space-x-4">
                          {application.aiAnalysis?.matchPercentage ? (
                            <div
                              className={`px-4 py-2 rounded-lg font-bold text-lg ${getScoreColor(application.aiAnalysis.matchPercentage)}`}
                            >
                          ...
                          <div
                            className={`px-4 py-2 rounded-full text-sm font-medium text-white ${getStatusColor(application.status)}`}
                          >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 pt-6 border-t border-white/10">
                            ...
                            <div className="space-y-2 text-sm">
                              <div className="flex justify-between">
                                ...
                                <span className="text-white font-bold">
                                  {application.aiAnalysis.atsScore?.overall ||
                                    0}
                                  %
                                </span>
                              </div>
                          ...
                          <div className="bg-white/5 rounded-xl p-4 border border-white/10">
                            ...
                            <div className="flex flex-wrap gap-1">
                              {(
                                application.aiAnalysis.skillsAnalysis
                                  ?.matchingSkills || []
                              )
                                .slice(0, 4)
                                .map((skill, i) => (
                          ...
                          <div className="bg-white/5 rounded-xl p-4 border border-white/10">
                            ...
                            <div className="text-sm">
                              <p className="text-gray-400">
                                Total:{" "}
                                <span className="text-white">
                                  {application.aiAnalysis.experienceAnalysis
                                    ?.totalYears || 0}{" "}
                                  yrs
                                </span>
                              </p>
                          ...
                          <div className="bg-white/5 rounded-xl p-4 border border-white/10">
                            ...
                            <div
                              className={`px-3 py-2 rounded text-sm font-medium text-center ${
                                application.aiAnalysis.hiringRecommendation ===
                                "strong-hire"
                      ...
                      <div className="flex flex-wrap gap-3 pt-6 border-t border-white/10 mt-6">
                        {application.applicant?.profile?.resume?.fileUrl && (
                          <a
                            href={
                              application.applicant.profile.resume.fileUrl.startsWith(
                                "http",
                              )
                                ? application.applicant.profile.resume.fileUrl
            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateCandidateField("name", e.target.value)
                      }
                      disabled={!editMode}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateCandidateField("email", e.target.value)
                      }
                      disabled={!editMode}
                      className="w-full px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-white mt-1 disabled:opacity-60"
                    />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateCandidateField("phone", e.target.value)
                      }
                      disabled={!editMode}
                      className="w-full px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-white mt-1 disabled:opacity-60"
                    />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateCandidateField("location", e.target.value)
                      }
                      disabled={!editMode}
                      className="w-full px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-white mt-1 disabled:opacity-60"
                    />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateCandidateField("currentRole", e.target.value)
                      }
                      disabled={!editMode}
                      className="w-full px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-white mt-1 disabled:opacity-60"
                    />
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={
                        currentCandidate.extractedInfo?.totalYearsExperience ||
                        ""
                      }
                      onChange={(e) =>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateCandidateField("education", e.target.value)
                      }
                      disabled={!editMode}
                      className="w-full px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-white mt-1 disabled:opacity-60"
                    />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateCandidateField("university", e.target.value)
                      }
                      disabled={!editMode}
                      className="w-full px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-white mt-1 disabled:opacity-60"
                    />
              ...
              <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
                ...
                <div className="flex flex-wrap gap-2">
                  {(currentCandidate.extractedInfo?.skills || []).map(
                    (skill, i) => (
                      <span
                        key={i}
                        className="px-3 py-1 bg-blue-500/20 text-blue-300 rounded-lg border border-blue-500/30"
                      >
                        {skill}
                      </span>
                    ),
